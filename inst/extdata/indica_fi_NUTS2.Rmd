...
mGallEU <- ggplot2::ggplot(wkDF3,aes(x = time, y= Indicator, 
                                      group=Country)) +
           ggplot2::geom_line() +  #no colors when 28+ teritories (the legend becomes massive!)
     ...
     geom_dl(aes(label = Country), method = list(dl.trans(x = x + 0.2), "last.points", cex = 0.5)) +
  geom_dl(aes(label = Country), method = list(dl.trans(x = x - 0.2), "first.points", cex = 0.5)) 
...
```
 ...
 <br>    
  * Regions where mean increased in year `r ptime_t` with respect  to year   `r ptime_0`:   
  
      + `r  paste(sort(dichia_con_stddev$res$declaration_split$names_incre),collapse="; ")`   
      
  * Regions where mean decreased in year `r ptime_t` with respect  to year   `r ptime_0`:    
...
```{r,echo=FALSE,results='asis',eval=TRUE, fig.width=6,fig.height=5}
if(semaforo[["sigma"]]){
...
The dynamics of regions show the differences in the situation of single
 regions which can be hidden under the use of a single indicator.
...
df2$label_change<-ifelse(df2$change<=-2,"-2 clusters or more",
                         ifelse(df2$change==-1,"-1 cluster",
                                ifelse(df2$change==0, "remained the same",
                                       ifelse(df2$change>=1 & df2$change<2,"+1 cluster", "+2 clusters or more"))))
...
write.csv(df2, "regions_cluster.csv", row.names = F)
msdf <- read.csv("regions_cluster.csv")
# #get rid of columns not needed
# msdf<-msdf[,-c(2:4)]
# names(msdf)[2]<-paste("Cluster in year ",time_0,sep="")
# names(msdf)[3]<-paste("Cluster in year ",time_t,sep="")
# names(msdf)[4]<-"Change"
# 
# ftab <- autofit(flextable(head(msdf,20))) 
# # ftab <- delete_part(ftab, part = "header")
# # ftab <- (ftab, values = c("ID", as.character(miniT), as.character(maxiT), "change", "label_t_0", "label_t_0", "label_change"))
# ftab <- theme_vanilla(ftab) 
# 
# ftab <- footnote(ftab,i = 1, j=1, part = 'header', value = as_paragraph(c("Please refer to regions_cluster.csv file for the complete dataset")), ref_symbols = "*") %>% width(., width = 1)
# 
# save_as_image(ftab,path = "memtab.png", zoom = 9, webshot = "webshot2")
# DT::datatable(df2, rownames = F)
```

Regions where change is -2 clusters:
```{r,echo=FALSE} 
# tab <- "memtab.png"
# include_graphics(tab)
msdf %>% 
  filter(label_change== "-2 cluster") %>%
  .[1] %>%
  c()
...
Regions where change is -1 clusters:

```{r,echo=FALSE} 
msdf %>% 
  filter(label_change== "+1 cluster") %>%
  .[1] %>%
  c()
...
```
* the full table with region clusters is in the fiche folder
...
<img src="eurofound.jpg" width=50px height=40px>
<br> 
```{r,echo=FALSE,results='hide',fig.keep='all',fig.width=6, fig.height=10}
...
#HERE STARTS the SECOND version of the flextable aka flx2
flx2 <- autofit(flextable(workMatSco2))
flx2 <- rotate(flx2,j = 2:(ncol(workMatSco2)-1), rotation="btlr",part="header")
...
##flextable::save_as_image(flx2,path = "flextab.png", zoom = 6, webshot = "webshot2")

#04/08/2023
save_as_html(flx2,path="flx2.html")
webshot2::webshot("flx2.html", file="flextab.png") #left, top, width, and height
...
geodata_countries <- get_eurostat_geospatial(output_class = "sf", 
                                             resolution = "60", 
                                             nuts_level = "2", 
                                             year = "2021", make_valid = FALSE)  ##n get_eurostat_geospatial(output_class = "sf", resolution = "10",  :
...
*These maps show the statistical values per region (NUTS2).  
